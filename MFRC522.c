...
static char *blockaddr2sectorblock(unsigned int blockaddr) {
  ...
  if (blockaddr < (32 * 4))
    snprintf(sector_block, sizeof(sector_block), "%u/%u", blockaddr / 4,
             blockaddr % 4);
  else if (blockaddr < (32 * 4 + (39 - 32 + 1) * 16)) {
    blockaddr -= 32 * 4;
    snprintf(sector_block, sizeof(sector_block), "%u/%u", 32 + (blockaddr / 16),
             blockaddr % 16);
  } else
 ...
 *	backLenp: address of unsigned int where to store length of received
 *response IN BITS! RETURNS: MI_XXX status
 */
...
int MFRC522_Anticoll(unsigned char **backDatap) {
   ...
   */
  status =
      MFRC522_ToCard(PCD_TRANSCEIVE, cmd, sizeof(cmd), &backData, &backLen);
...
int MFRC522_SelectTag(unsigned char *serNum) {
  ...
  status =
      MFRC522_ToCard(PCD_TRANSCEIVE, buf, sizeof(buf), &backData, &backLen);
 ...
 */
void MFRC522_StopCrypto1() { ClearBitMask(Status2Reg, 0x08); }
...
void MFRC522_Read(unsigned char blockAddr) {
  ...
  status = MFRC522_ToCard(PCD_TRANSCEIVE, recvData, sizeof(recvData), &backData,
                          &backLen);
...
void MFRC522_Write(unsigned char blockAddr, unsigned char *writeData,
  ...
  status =
      MFRC522_ToCard(PCD_TRANSCEIVE, buff, sizeof(buff), &backData, &backLen);
  ...
  if (status == MI_OK) {
    ...
    status = MFRC522_ToCard(PCD_TRANSCEIVE, buf, writeDataLen + 2, &backData,
                            &backLen);
    if ((status != MI_OK) || (backLen != 4) || ((backData[0] & 0x0F) != 0x0A))
